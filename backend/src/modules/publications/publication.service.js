 ...
 */
export const getPublications = async ({ contentId, platform, page = 1, limit = 20 }) => {
  const query = {};
...
export const createPublication = async (data, userId) => {
  ...
  const pub = await Publication.create({
    contentId, platform, publishedAt, postUrl, notes,
    publishedBy: userId,
  });
  ...
  if (content.status === CONTENT_STATUSES.SCHEDULED || content.status === CONTENT_STATUSES.APPROVED) {
...
export const updatePublication = async (id, data, userId) => {
  const pub = await Publication.findByIdAndUpdate(id, data, { new: true, runValidators: true })
    .populate('contentId', 'title contentType')
    .populate('publishedBy', 'name email');
