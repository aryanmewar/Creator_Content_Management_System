...
export const getSavedLinks = async (req, res, next) => {
  try {
    const savedLinks = await SavedLink.find({ createdBy: req.user._id }).sort('-createdAt');
...
export const updateSavedLink = async (req, res, next) => {
  try {
    ...
    if (!savedLink) {
      return sendError(res, { message: 'No saved link found with that ID', statusCode: 404 });
    }
...
export const deleteSavedLink = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, { message: 'Deleted successfully', statusCode: 200 });
  } catch (error) {
    next(error);
  }
};
