...
export const createSchedule = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, { message: 'Content scheduled successfully.', data: schedule, statusCode: 201 });
  } catch (error) { next(error); }
};

export const updateSchedule = async (req, res, next) => {
  try {
    const schedule = await scheduleService.rescheduleContent(req.params.id, req.body, req.user._id);
    return sendSuccess(res, { message: 'Content rescheduled successfully.', data: schedule });
  } catch (error) { next(error); }
};
