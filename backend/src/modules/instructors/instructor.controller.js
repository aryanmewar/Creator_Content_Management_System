...
export const getInstructors = async (req, res, next) => {
  try {
    ...
    const result = await instructorService.getInstructors({ search, isActive, page, limit });
...
export const createInstructor = async (req, res, next) => {
  try {
    let imageData = null;
    if (req.uploadedFile) {
      imageData = { url: req.uploadedFile.url, publicId: req.uploadedFile.publicId };
    }
    const data = { ...req.body, ...(imageData && { profileImage: imageData }) };
    const instructor = await instructorService.createInstructor(data, req.user._id);
    return sendSuccess(res, { message: 'Instructor created successfully.', data: instructor, statusCode: 201 });
  } catch (error) { next(error); }
};
...
export const updateInstructor = async (req, res, next) => {
  try {
    ...
    const data = { ...req.body, ...(imageData && { profileImage: imageData }) };
    const instructor = await instructorService.updateInstructor(req.params.id, data, req.user._id);
    return sendSuccess(res, { message: 'Instructor updated successfully.', data: instructor });
  } catch (error) { next(error); }
};
...
export const updateInstructorStatus = async (req, res, next) => {
  try {
    ...
    const instructor = await instructorService.updateInstructorStatus(req.params.id, isActive, req.user._id);
