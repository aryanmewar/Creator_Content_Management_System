...
const AssignContent = () => {
  ...
  const { contents, isLoading, filters, statusCounts, dispatch, fetchContent } =
    useContentContext();
  ...
  useEffect(() => {
    ...
    const params =
      activeTab === "All"
        ? { ...filters, status: "", hasContributors: true, dateFrom, dateTo }
        : {
            ...filters,
            status: activeTab,
              ...
              {STATUS_TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded-full text-[10px] ${activeTab === tab ? "bg-primary/10 text-primary" : "bg-slate-200/50 text-slate-500"}`}
                    >
                      {count}
                    </span>
                  </button>
