...
const COLORS = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#60a5fa', '#a78bfa', '#34d399', '#f87171'];
      ...
      try {
        ...
        (allContent.data || []).forEach((c) => {
          ...
          if (c.status === 'PUBLISHED') {
            ...
            if (c.contributors?.length > 0) byWhom = c.contributors.map(cont => cont.name).join(', ');
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatsCard title="Total Content" value={summary?.totalContent} icon={Target} color="primary" />
        <StatsCard title="Published" value={summary?.published} icon={Award} color="success" />
        <StatsCard title="Overdue" value={summary?.overdue} icon={TrendingUp} color="danger" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card p-6 xl:col-span-2">
          ...
          {deliveredContent.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-8">No delivered content available</p>
          ) : (
            <>
              {/* Desktop Table */}
                  ...
                  <tbody>
                    {deliveredContent.map((item, i) => (
                      <tr key={i} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-slate-600">{item.byWhom}</td>
                        <td className="py-3 px-4 text-slate-600">{item.publishedDate ? new Date(item.publishedDate).toLocaleDateString() : 'N/A'}</td>
                        <td className="py-3 px-4 text-slate-600">{item.platforms}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="lg:hidden flex flex-col gap-3">
                {deliveredContent.map((item, i) => (
                  <div key={i} className="p-4 rounded-xl border border-slate-100 bg-slate-50">
                    ...
                    <p className="text-xs text-slate-500 mb-2">By: <span className="font-medium text-slate-700">{item.byWhom}</span></p>
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-200">
                      <p className="text-xs text-slate-500">{item.publishedDate ? new Date(item.publishedDate).toLocaleDateString() : 'N/A'}</p>
