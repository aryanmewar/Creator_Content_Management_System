...
const Instructors = () => {
  const { instructors, isLoading, fetchInstructors, dispatch } =
  ...
  const handleCreate = async (formData) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to create contributor.",
      );
    } finally {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      const response = await instructorService.updateStatus(
        toggleTarget._id,
        newStatus,
      );
      dispatch({ type: "UPDATE_INSTRUCTOR", payload: response.data });
      toast.success(
        `Contributor ${newStatus ? "activated" : "deactivated"} successfully.`,
      );
  ...
  const handleDelete = async () => {
    ...
    setConfirmLoading(true);
    try {
      await instructorService.deleteInstructor(deleteTarget._id);
      dispatch({ type: "REMOVE_INSTRUCTOR", payload: deleteTarget._id });
  ...
  const stats = useMemo(() => {
    return {
      ...
      assignments: instructors.reduce(
        (acc, curr) => acc + (curr.stats?.total || 0),
        0,
      ),
    };
  }, [instructors]);
    ...
    <>
      <div className="font-sans">
        ...
        <ConfirmDialog
          ...
          message={`Are you sure you want to permanently remove "${deleteTarget?.name}"? This action cannot be undone and will delete their user account.`}
          confirmLabel="Remove"
          variant="destructive"
        />
      </div>
