...
const instructorReducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_SELECTED":
      return { ...state, selectedInstructor: action.payload, isLoading: false };
    case "ADD_INSTRUCTOR":
      return { ...state, instructors: [action.payload, ...state.instructors] };
    case "UPDATE_INSTRUCTOR":
      return {
        ...
        selectedInstructor:
          state.selectedInstructor?._id === action.payload._id
            ? action.payload
    ...
    case "REMOVE_INSTRUCTOR":
      return {
        ...state,
        instructors: state.instructors.filter((i) => i._id !== action.payload),
        selectedInstructor:
          state.selectedInstructor?._id === action.payload
            ? null
            : state.selectedInstructor,
      };
    default:
...
export const InstructorProvider = ({ children }) => {
  ...
  const fetchInstructors = useCallback(async (params = {}) => {
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: err.response?.data?.message || "Failed to load instructors.",
      });
    }
  }, []);
  ...
  const fetchInstructorById = useCallback(async (id) => {
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: err.response?.data?.message || "Failed to load instructor.",
      });
    }
  }, []);
