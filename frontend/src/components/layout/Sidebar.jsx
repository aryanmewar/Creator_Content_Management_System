    ...
    <>
      ...
      >
        ...
        <div className="px-4 py-5 border-b border-white/10 flex items-center relative">
          <div className={`flex items-center gap-3 overflow-hidden transition-all duration-500 ease-out ${isVisualCollapsed ? 'max-w-0 opacity-0' : 'max-w-[160px] opacity-100'}`}>
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shrink-0 shadow-sm">
              <Zap className="w-5 h-5 text-white shrink-0" />
            </div>
            <div className="flex flex-col">
              <p className="text-white font-logo text-xl leading-tight truncate tracking-wide">Createlyt</p>
              <p className="text-slate-400 text-[10px] uppercase tracking-wider">Workspace</p>
            </div>
          </div>
          <Button
            variant="ghost"
          ...
          >
            {isVisualCollapsed ? <ChevronRight className="w-5 h-5" /> : <ChevronLeft className="w-5 h-5" />}
          </Button>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto">
          {navItems.map(({ to, icon: Icon, label }) => (
            ...
            >
              <Icon className={`w-5 h-5 shrink-0 transition-all duration-500 ease-out ${isVisualCollapsed ? '' : 'mr-3'}`} />
              <span className={`overflow-hidden whitespace-nowrap transition-all duration-500 ease-out ${isVisualCollapsed ? 'max-w-0 opacity-0' : 'max-w-[200px] opacity-100'}`}>{label}</span>
            </NavLink>
          ))}
        </nav>
        ...
        <div className="px-3 py-4 border-t border-white/10">
          {/* User info */}
          <div className={`flex items-center h-10 mb-2 relative ${isVisualCollapsed ? 'justify-center' : 'px-3'}`}>
            <div className={`w-8 h-8 rounded-full bg-primary flex items-center justify-center shrink-0 transition-all duration-500 ease-out ${isVisualCollapsed ? '' : 'absolute left-3'}`} title={user?.name}>
              <span className="text-white text-xs font-semibold">
                {getInitials(user?.name || 'U')}
              </span>
            </div>
            <div className={`min-w-0 overflow-hidden whitespace-nowrap transition-all duration-500 ease-out ${isVisualCollapsed ? 'max-w-0 opacity-0' : 'ml-11 max-w-[150px] opacity-100'}`}>
              <p className="text-white text-sm font-medium truncate">{user?.name || 'User'}</p>
              <p className="text-slate-400 text-[10px] uppercase tracking-wider truncate">{user?.role?.replace('_', ' ')}</p>
            </div>
          </div>
          ...
          >
            <LogOut className={`w-5 h-5 shrink-0 transition-all duration-500 ease-out ${isVisualCollapsed ? '' : 'mr-3'}`} />
            <span className={`overflow-hidden whitespace-nowrap transition-all duration-500 ease-out ${isVisualCollapsed ? 'max-w-0 opacity-0' : 'max-w-[150px] opacity-100'}`}>Logout</span>
          </button>
        </div>
      </aside>
    </>
