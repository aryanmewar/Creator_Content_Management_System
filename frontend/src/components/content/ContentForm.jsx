...
const getSchema = (isOwnerView) => z.object({
  title: z.string().min(2, 'Title must be at least 2 characters').max(200),
  referenceLink: isOwnerView
    ? z.string().url('Must be a valid URL').optional().or(z.literal(''))
    : z.string().url('Must be a valid URL').min(1, 'Reference link is required'),
  ...
  dueDate: isOwnerView
    ? z.string().optional().refine(val => !val || !isNaN(new Date(val).getTime()), { message: 'Invalid due date' })
    : z.string().min(1, 'Due date is required').refine(val => !isNaN(new Date(val).getTime()), { message: 'Invalid due date' }),
  completionDate: z.string().refine(val => !isNaN(new Date(val).getTime()), { message: 'Invalid date' }),
  notes: z.string().max(2000).optional(),
});

const ContentForm = ({ isOpen, onClose, onSubmit, content = null, isLoading = false, instructors = [], hideDueDate = false, isOwnerView = false }) => {
  ...
  useEffect(() => {
    if (content) {
      const dateStr = content.completionDate ? new Date(content.completionDate).toISOString().split('T')[0] : '';
      const dueStr = content.dueDate ? new Date(content.dueDate).toISOString().split('T')[0] : '';
      ...
      reset({
        ...
        contentType: Array.isArray(content.contentType) ? content.contentType : (content.contentType ? [content.contentType] : []),
  ...
  const contributorOptions = instructors.map((i) => ({ label: i.name, value: i._id }));
      ...
      <DialogContent className="sm:max-w-3xl p-8 bg-white shadow-2xl rounded-[32px] border border-slate-100">
        <DialogHeader className="mb-6">
          ...
          <DialogDescription className="text-sm font-medium text-slate-500 mt-1">
            {content ? 'Update the details of your content below.' : 'Fill out the form below to create a new piece of content.'}
          </DialogDescription>
        </DialogHeader>

        <form id="glass-content-form" onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                ...
                />
                {errors.title && <p className="text-xs font-medium text-rose-500 px-1">{errors.title.message}</p>}
              </div>

              <div className="space-y-1.5">
                <label className={labelClasses}>
                  Reference Link {!isOwnerView && <span className="text-rose-500">*</span>}
                </label>
                <input
                  type="url"
                ...
                />
                {errors.referenceLink && <p className="text-xs font-medium text-rose-500 px-1">{errors.referenceLink.message}</p>}
              </div>
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className={labelClasses}>
                  {isOwnerView ? "Other Contributors" : "Contributors"} {!isOwnerView && <span className="text-rose-500">*</span>}
                </label>
                <Controller
                  name="contributors"
                  control={control}
              ...
              <div className="grid grid-cols-2 gap-4">
                {!hideDueDate && (
                  <div className="space-y-1.5">
                    <label className={labelClasses}>
                      Due Date {!isOwnerView && <span className="text-rose-500">*</span>}
                    </label>
                    <input
                      type="date"
                      {...register('dueDate')}
                    ...
                    />
                    {errors.dueDate && <p className="text-xs font-medium text-rose-500 px-1">{errors.dueDate.message}</p>}
                  </div>
                )}

                <div className={`space-y-1.5 ${hideDueDate ? 'col-span-2' : ''}`}>
                  <label className={labelClasses}>
                  ...
                  />
                  {errors.completionDate && <p className="text-xs font-medium text-rose-500 px-1">{errors.completionDate.message}</p>}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  {...register('notes')}
                />
                {errors.notes && <p className="text-xs font-medium text-rose-500 px-1">{errors.notes.message}</p>}
              </div>
            </div>
          </div>
        </form>
        ...
        <div className="mt-8 pt-4 flex sm:justify-end gap-3 items-center border-t border-slate-100">
          ...
          >
            {isLoading ? 'Processing...' : (content ? 'Save Changes' : 'Create Content')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
